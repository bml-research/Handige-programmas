...
student_answers <- function(file_to_read) {
  file_to_read <- readWorkbook(paste0(save_path, "/Computervaardigheden_Eindopdracht.xlsx"), 
                               sheet = 1)
...
nakijken <- function(input_students, answers) {
  ...
  new_df4 <- cbind(input_students[, c(14, 3, 15, 16, 1, 2)], new_df3)
...
write_file <- function(nagekeken) {
  file_name <- paste0(save_path, "/nakijk_eindopdracht_origineel.xlsx")
