...
save_path <- "/Users/markuiz/PycharmProjects/Eigen projectjes/Nakijken_Bioinformatica/nakijken_inhaallessen/"
...
student_answers <- function(file_to_read) {
  ...
  students_df <- file_to_read[, c(2,3,5:(ncol(file_to_read) - 5), 
                                  52:56)]
  students_df$Start.time <- strftime(convertToDateTime(students_df$Start.time), 
                                     format = "%d/%m/%Y %H:%M:%S")
  students_df$Completion.time <- strftime(convertToDateTime(students_df$Completion.time), 
             format = "%d/%m/%Y %H:%M:%S")
...
nakijken <- function(input_students, answers) {
  ...
  new_df4 <- cbind(input_students[, c(50, 3, 51:54, 1, 2)], new_df3)
...
write_file <- function(nagekeken) {
  ...
  conditionalFormatting(new_wb, 1, cols = 10,
