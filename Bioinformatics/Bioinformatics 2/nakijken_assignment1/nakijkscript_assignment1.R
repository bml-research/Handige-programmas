...
student_answers <- function(file_to_read) {
  file_to_read <- readWorkbook(paste0(save_path, "/answers_students_assignment1_inhaal.xlsx"), 
                               sheet = "Form1")
...
nakijken <- function(input_students, answers) {
  ...
  new_df4 <- cbind(input_students[, c(29, 3, 30, 31, 1, 2)], new_df3)
...
write_file <- function(nagekeken) {
  file_name <- paste0(save_path, "/final_nakijk_assignment1.xlsx")
