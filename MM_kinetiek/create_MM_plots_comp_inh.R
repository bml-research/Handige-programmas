...
create_data <- function(km) {
  ...
  km_inh <- km * (2+(runif(1, 0.1, 0.5)))
  answers <- c(vmax, vmax, km, km_inh)
  ...
  answers_df$Answer <- "A) Competitieve inhibitor; Vmax blijft gelijk, Km verandert"
  
  # create data from random value of Km
  conc_S <- answers[3] * c(0.5, 1.0, 2.0, 4.0, 6.0, 10.0)
  vel <- c(answers[1] * (conc_S/(answers[3] + conc_S)))
  vel_inh <- c(answers[1] * (conc_S/(answers[4] + conc_S)))
  ...
  plot_LB <- ggplot(data = LB_df_melted, aes(x = `1/[S] (1/mM)`, y = value)) +
    ...
    geom_abline(intercept = 1 / answers[1], 
                slope = answers[4] / answers[1], colour = "blue") +
    ...
    stat_poly_eq(data = LB_df, formula = y ~ x, 
                 ...
                 parse = TRUE, color = "blue", label.y = 0.85, label.x = 0.95)
...
create_files <- function(test_df) {
  #location of files
  save_path <- paste0(work_path,"/MM_data/Comp")
  ...
  #### kan eventueel ook nog een template maken ####
  
  
  # write files with data + extras ('invulformulier')  
  names(data_list) <- paste0("dataset", 1L:length(data_list))
  for(s in seq_along(names(data_list))) {
    data_file_name = paste0(save_data, names(data_list[s]), ".xlsx")
    wb <- openxlsx::loadWorkbook(paste0(save_path, "/template_MM_comp.xlsx"))
    
    name_sheet <- paste0("Dataset", s)
  ...
  # write file with answers to all data sets
  write.xlsx2(all_answers, paste0(save_answers, "all_answers_comp.xlsx"),
              col.names = TRUE)
  ...
  names(answers_list) <- paste0("answers", 1L:length(answers_list))
  
  # writing data and answers 1 file (for all data sets)
  for(k in seq_along(names(answers_list))) {
    file_name = paste0(save_answers, "answers", k, ".xlsx")
